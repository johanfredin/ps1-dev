...
#define PAD_TYPE_MOUSE 0x1
#define PAD_TYPE_NAMCO_NEGCON 0x2
#define PAD_TYPE_DIGITAL 0x4
#define PAD_TYPE_ANALOG 0x5
#define PAD_TYPE_DUAL_SHOCK 0x7
#define PAD_TYPE_NAMCO_JOCCON 0xE
...
void check_pad(PadType *pad) {
    ...
    if (is_connected) {
        ...
        } if (!(pad->curr_btn & PAD_CROSS)) {
            current_button = "X";
        } if (!(pad->curr_btn & PAD_SQUARE)) {
            current_button = "[]";
        } if (!(pad->curr_btn & PAD_CIRCLE)) {
            current_button = "()";
        } if (!(pad->curr_btn & PAD_TRIANGLE)) {
            current_button = "/\\";
        } if (!(pad->curr_btn & PAD_START)) {
            current_button = "START";
        } if (!(pad->curr_btn & PAD_SELECT)) {
            current_button = "SELECT";
        } if (!(pad->curr_btn & PAD_R1)) {
            current_button = "R1";
        } if (!(pad->curr_btn & PAD_R2)) {
            current_button = "R2";
        } if (!(pad->curr_btn & PAD_L1)) {
            current_button = "L1";
        } if (!(pad->curr_btn & PAD_L2)) {
            current_button = "L2";
        }  if (!(pad->curr_btn & PAD_R3)) {
            current_button = "R3";
        }  if (!(pad->curr_btn & PAD_L3)) {
            current_button = "L3";
        }
    }
