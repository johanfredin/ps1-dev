...
Image createImage(unsigned char imageData[]) {
	...
	image.sprite.w = image.tim_data.pw * 2;         	// width of image
	image.sprite.h = image.tim_data.ph;             // height of image
    ...
    image.sprite.u=(image.tim_data.px - 320) * 2;                               // position within timfile for image
    image.sprite.v=image.tim_data.py;								// position within timfile for image
...
Box createBox(Color color, int x1, int y1, int x2, int y2) {
    ...
    box.line[2] = left;
    box.line[3] = right;
    printf("line coords: { x1:%d, y1:%d, x2:%d, y2%d: }\n", x1, y1, x2, y2);
    return box;
}
