...
void chdlr_collision_resolve(GameObject *gobj, RECT *level_bounds, u_char blocks_amount) {
    ...
    for (i = 0; i < blocks_amount; i++) {
        ...
        if (chdlr_overlaps(&gobj_bounds, &rect)) {
            ...
            short bottom =      (short) (gobj->sprite->y + gobj_bounds.h);
            short oldBottom =   (short) (gobj->old_y + gobj_bounds.h);
            short oldRight =    (short) (gobj->old_x + gobj_bounds.w);
            short rightBottom = (short) (rect.y + rect.h);

            if (right >= rect.x & oldRight <= rect.x) {                                          // From right
                gobj->sprite->x = (short) (rect.x - gobj->bounds->w);
            } else if (gobj->sprite->x <= (rect.x + rect.w) & old_x >= (rect.x + rect.w)) {      // From left
                gobj->sprite->x = (short) (rect.x + rect.w);
            } else if (bottom >= rect.y & oldBottom <= rect.y) {                                 // From above
                gobj->sprite->y = (short) (rect.y - gobj->bounds->h);
