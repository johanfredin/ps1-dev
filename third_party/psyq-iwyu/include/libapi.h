 ...
 * Format” of Chapter 13 (Controller/Peripherals Library) of the Library Overview.<p>
 * Since it is possible for mistakes to occur when an unexpected controller is connected to the receive data
 * length, always allocate 34 bytes.<p>
 * When using the Multi Tap, use InitTAP(). When using the gun controller, use InitGUN().
 * @param bufA pointer to incoming data buffer 1
 * @param bufB pointer to incoming data buffer 2
 * @param lenA length of incoming data buffer 1 (in bytes)
 * @param lenB length of incoming data buffer 2 (in bytes)
 * @return Always 1
 */
extern long InitPAD(char *bufA, long lenA, char *bufB, long lenB);

/**
 * Start reading the controller.<p>
...
/**
 * Set the control driver.<p>
 * if val is 1, interrupt processing in a control driver started by a vertical retrace line interrupt is completed. <br>
 * If val is 0, processing is passed to a lower priority interrupt module without completion.
