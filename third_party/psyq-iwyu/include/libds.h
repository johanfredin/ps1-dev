 ...
 * DsInit() needs to be called just once at the beginning of a program or when restarting a system that was
 * stopped with DsClose().
 * Calling DsInit() in the middle of a program may cause improper operation. DsReset() should be used if
 * initialization needs to be performed in the middle of a program.
 * Because DsInit() resets the SPU sound volume and the CD input volume to SPU, etc., it should either be
 * called before libspu and libsnd initialization/setting functions or they should be reset after DsInit() is called.
 ...
 * when CD control functions outside of libds are used. Call DsInit() to reopen libds.
 */
void DsClose(void);
 ...
 * @param mode Mode
 * @param tracks Pointer to array specifying the tracks to be played back; the last element of the array must be 0.
 * @param offset Index for track to begin playback
 * @return The track currently being played (the index in the tracks array rather than the absolute track number).
